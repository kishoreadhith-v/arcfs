...
Application-level deduplicators such as Restic perform
...
A write-back page cache—a
\texttt{HashMap\textless{}u64,\;(Vec\textless{}u8\textgreater{},\;bool)\textgreater{}}
bounded at 1{,}024 entries by an LRU eviction policy—coalesces small
...
data. The subsections below describe each layer in detail; benchmark
evidence appears in Section~\ref{sec:eval}.
...
overhead. It is typed as
\texttt{HashMap\textless{}u64,\;(Vec\textless{}u8\textgreater{},\;bool)\textgreater{}}:
the \texttt{u64} key is the inode identifier, the \texttt{Vec\textless{}u8\textgreater{}}
value accumulates the full write buffer for that inode, and the
\texttt{bool} flag records whether the entry is dirty (i.e., unflushed
to the CAS layer). A FUSE write call inserts or updates the target
inode's entry, appends incoming bytes to its buffer, and sets the dirty
flag to \texttt{true} before returning success to the kernel; no disk
I/O occurs on the acceptance path. The physical pipeline
(FastCDC $\to$ SHA-256 $\to$ zstd $\to$ CAS) activates at one of three
points: the cache reaches its 1{,}024-entry LRU capacity and evicts the
least-recently-used inode's entry, the application calls \texttt{fsync(2)},
or the kernel calls \texttt{release} on file close. This deferred-coherence
model shifts the throughput ceiling from physical media allocation to FUSE
IPC bandwidth.
...
until a garbage collection pass scans all live \texttt{ino\_recipe} entries
to compute a reachable chunk set and removes any CAS blob not referenced
by at least one recipe or snapshot.
...
\texttt{ino\_recipe:\textless{}id\textgreater{}} & Ordered chunk hash list + file size \\
...
old one. Because GC derives reachability by scanning all live and snapshot
recipes, a chunk shared between the live tree and a snapshot is never
reclaimed until both references are removed.
...
line-oriented commands without requiring an unmount cycle:
\begin{lstlisting}
# tag a file while the filesystem is live
echo "set docs/report.txt work 2026" \
     > mnt/.tags/.tagfs_control
# remove all tags for a path
echo "del docs/report.txt" \
     > mnt/.tags/.tagfs_control
\end{lstlisting}
The command format is \texttt{\textless{}verb\textgreater{} \textless{}rel-path\textgreater{} [tags\ldots]};
\texttt{set} atomically replaces the tag list, and \texttt{del} removes
all tags for the given path.
...
to 4\,GB of incompressible data. The 1{,}024-entry LRU cache absorbed
